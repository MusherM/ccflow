...
test("deleting through empty ancestors is valid and resolves reset commit recursively", () => {
  ...
  assert.equal(result.focusId, "node_empty");
  assert.equal(state.nodes.node_leaf, undefined);
  assert.equal(state.nodes.node_empty?.type, "leaf");
  assert.equal(state.nodes.node_empty?.git.commitHash, null);
  assert.equal(state.currentNodeId, "node_empty");
  assertGraphInvariants(state);
});
